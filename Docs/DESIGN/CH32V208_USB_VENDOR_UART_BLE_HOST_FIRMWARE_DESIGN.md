...
本项目后续实施过程中，测试和验证遵循以下约束：

- 不要求为本项目额外落盘测试代码
- 不要求新增单元测试源码、自动化测试脚本或专用测试工程文件
- 需要在实施计划和执行过程中明确列出每个阶段的验证内容、预期现象和判定标准
- 后续优先通过 MCU 运行日志、串口打印、USB 交互结果、BLE 状态变化和用户现场观察来验证效果
- 在关键实现节点，如果当前环境无法自行闭环验证，应暂停并明确告诉用户需要验证的现象和操作步骤
...
如果后续技能支持直接获取 MCU 日志，则优先基于日志验证阶段性结果；若日志无法覆盖结论，则需要停在对应节点并提示用户进行现场确认。

### 16.3 停点策略

以下情况需要在执行中显式停下并通知用户验证：

- USB 已完成枚举，但 Host 侧串口对象是否按预期出现需要用户确认
- UART 映射与目标硬件引脚行为需要用户确认
- BLE 扫描或连接结果依赖现场设备环境
- BLE 服务发现、通知订阅等行为需要真实外设配合
- 后续 ETH 扩展接入时需要用户确认链路和网络现象
...
虽然不落盘测试代码，但后续实施计划中仍必须明确写出测试用例语义，至少包括：
...
这些测试用例以“验证项清单”的形式存在，而不是以源码测试文件形式存在。
